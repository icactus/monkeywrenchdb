...
def banded_dtw(x, y, window):
    ...
    # Numba can't return lists of lists easily to python without typed lists, but returning zip-like tuples works.
    # Actually just building 2 lists and turning to arr is fine, but to output python list of tuples like dtaidistance:
...
f1 = np.random.rand(40000, 195)
f2 = np.random.rand(40000, 195)
t0 = time.time()
path = banded_dtw(f1, f2, window=650)
print(time.time()-t0, path.shape)
