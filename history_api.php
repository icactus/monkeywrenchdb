...
try {
    ...
    if ($action === 'add') {
        ...
        if ($json === false) {
            ...
            error_log('[history_api] JSON encode failed: ' . json_last_error_msg());
    ...
    http_response_code(500);
    ob_end_clean();
    error_log(sprintf('[history_api] %s in %s:%d', $e->getMessage(), $e->getFile(), $e->getLine()));
    echo json_encode(['error' => 'Server error']);
