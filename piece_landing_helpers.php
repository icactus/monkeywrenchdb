...
function mwResolvePieceLandingBySlug(mysqli $conn, string $slug): ?array
{
    ...
    $pieces = mwGetPieceLandingCandidates($conn);
    $match = null;
    ...
    $metricStmt = $conn->prepare("
        ...
        LIMIT 1
    ");
    $metricStmt->bind_param('i', $match['piece_id']);
    $metricStmt->execute();
    ...
    $recordingStmt = $conn->prepare("
        ...
        LIMIT 1
    ");
    $recordingStmt->bind_param('i', $match['piece_id']);
    ...
    return [
        ...
        'piece_name' => $match['piece_name'],
        'composer_last' => $match['composer_last'],
        'piece_slug' => $match['piece_slug'],
        'piece_path' => $match['piece_path'],
        'piece_url' => mwGetBaseUrl() . $match['piece_path'],
