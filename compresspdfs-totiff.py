    ...
    if filename.lower().endswith('.pdf'):
        ...
        for page_filename in os.listdir(temp_directory):
            if page_filename.startswith(image_base_name) and page_filename.endswith('.pdf'):
                ...
                subprocess.run([magick_path, '-verbose', '-density', '600', pdf_page_path, '-background', 'white', '-alpha', 'remove', '-quality', '100', tiff_output_filename], check=True)
                subprocess.run([magick_path, '-verbose', '-density', '300', tiff_output_filename, '-deskew', '40%', '+repage', '-resize', '2000x', '-monochrome', '-compress', 'Group4', tiff_output_filename], check=True)
        ...
        print(f"Combining TIFF files into {processed_pdf_path}")
        subprocess.run(f"{magick_path} {temp_directory}/*.tif {processed_pdf_path}", shell=True, check=True)
