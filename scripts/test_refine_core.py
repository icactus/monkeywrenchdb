...
def run():
    ...
    # Simulated values corresponding to Mix 5 for Rec1
    t1_sec = 6.766
    
    # We will simulate the same coarse estimate that the pipeline got
    # For now, let's just try t2_est_sec around 8.406s
    t2_est_sec = 8.406
    
    t2_refined, feat_dist = _local_refine_core(f1_hires, f2_hires, t1_sec, t2_est_sec, window_sec=5.0, local_hop=256, sr=syncer.sr)
    
    print(f"t1={t1_sec}s -> t2_refined={t2_refined:.3f}s, feature_distance={feat_dist:.4f}")
