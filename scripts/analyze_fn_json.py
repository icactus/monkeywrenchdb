...
# Read the full evaluation results instead, which contains confidence/feature info 
# rather than the raw sync path.
...
false_negatives = []
# The user's log indicates 362 unflagged items where they had GT > 0.15s
# We don't have the explicit user GT here, but we can look at the general distribution
# of points that were marked 'HIGH'
...
for fd_th in [1.05, 0.95, 0.85, 0.75, 0.65, 0.55]:
