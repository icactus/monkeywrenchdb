...
// Removed mixer related variables and functions as the mixer UI and functionality are removed.
// Keeping hasMixer$$module$synpdf and dummyPlayer$$module$synpdf as they are used in core logic.
var playing$$module$mixer = 0,
    ...
    tracksOn$$module$mixer, tracksOnPrev$$module$mixer, soloTrk$$module$mixer, startTime$$module$mixer, playPos$$module$mixer =
        0,
    // playbtn$$module$mixer, // Removed play button variable
    posElm$$module$mixer, progressElm$$module$mixer, nframes$$module$mixer = 0,
    animFrame$$module$mixer, vumax$$module$mixer = 0,
    duration$$module$mixer, wasplaying$$module$mixer = 0;
// Removed unused mixer HTML templates and CSS variable

function logerr$$module$mixer(a) {
    // Check if the error element exists before appending
    const errElm = document.getElementById("err");
    if (errElm) {
        errElm.innerText += a + "\n";
    } else {
        console.error("Error element #err not found:", a);
    }
}

// Removed unused mixer functions: readAudio$$module$mixer, mkAudBuffers$$module$mixer, startstop$$module$mixer, peakMeter$$module$mixer, setButtons$$module$mixer, mute$$module$mixer, solo$$module$mixer, setvol$$module$mixer, setmstr$$module$mixer, mutemstr$$module$mixer, setpan$$module$mixer, setTrkNames$$module$mixer, setTrkHandlers$$module$mixer, setTrkOrient$$module$mixer, initMixer$$module$mixer, mixplayer$$module$mixer
...
var msc_VERSION$$module$synpdf = 182,
    ...
    hasMixer$$module$synpdf = 0, // Keep this, but mixer functions removed. Will default to dummy player.
    ...
    opt_default$$module$synpdf = {
        ...
        delay: 0,
        ipaddr: "",
        mstr: 0, // Related to mixer, might keep but won't do anything
        bpmsr: "4-20-1",
        loop: 0,
        annot: 0,
        zwgrens: .7,
        ...
        fscr: 0,
        pagenum: 1,
        playbtn: 0, // Related to play button, will be removed
        mmin: "",
        fixwd: 1E3,
        lastSynced: -2,
        eerst: 0,
...
function initPreload$$module$synpdf() {
    ...
    // Removed line hiding yvdlbl and vidyub here, will handle visibility in medbtnSwitch
    //$("#yvdlbl, #vidyub").css("display", "none");
    msc_credits$$module$synpdf = void 0;
    $("#credits").html("");
    // Removed metric_arr, pdf_data, tix_lb, times_arr as they are re-initialized later or fetched from DB
    ...
    // Removed line hiding/showing #buttons height as #buttons structure changed and file buttons are removed
    // $("#buttons").css("height", "");
    pdf_file$$module$synpdf = media_file$$module$synpdf = pdfFnm$$module$synpdf = mediaFnm$$module$synpdf =
        "";
    yubchk$$module$synpdf = 0;
    elmed$$module$synpdf = null; // Ensure elmed is null initially
    annots$$module$synpdf = [];
    initLoopRec$$module$synpdf();
    deMetriek$$module$synpdf[0] = opt$$module$synpdf.pagewd // Initialize deMetriek
}
...
function initGlobals$$module$synpdf() {
    ...
    jpgData$$module$synpdf = pdfData$$module$synpdf = null; // Keep these for potential PDF data saving
    // Ensure deTijden is initialized correctly based on times_arr
    deTijden$$module$synpdf = times_arr$$module$synpdf ? times_arr$$module$synpdf : [];
    // Check if deTijden$$module$synpdf is an array of arrays and flatten it
    if (deTijden$$module$synpdf.length > 0 && Array.isArray(deTijden$$module$synpdf[0])) {
        deTijden$$module$synpdf = deTijden$$module$synpdf.reduce(function(a, b) {
            return a.concat(b.slice(1)) // Assuming the first element of inner arrays is not needed for times
        }, []);
    }
    // Ensure deTijden$$module$synpdf has the correct structure {t: time, mix: measureIndex}
    deTijden$$module$synpdf = deTijden$$module$synpdf.map(function(a, b) {
        // Assuming 'a' might be just the time value if already flattened once, or an array element
        const time = typeof a === 'object' && a !== null && a.t !== undefined ? a.t : a;
        const mix = typeof a === 'object' && a !== null && a.mix !== undefined ? a.mix : b; // Use existing mix or index
        return { t: time, mix: mix };
    });
    ...
    repMaten$$module$synpdf = [] // Keep repeat measures array
}
...
Wijzer$$module$synpdf.prototype.drawRepTokens = function() {
    function a(a, b, c) {
        ...
        b = c[a] || 1;
        c[a] = b + 1;
        // Check if deMaten$$module$synpdf[a] exists before accessing properties
        if (!deMaten$$module$synpdf[a]) {
            console.warn("deMaten$$module$synpdf[" + a + "] is undefined.");
            return; // Skip drawing token if measure data is missing
        }
        a = deMaten$$module$synpdf[a];
        // Check if $("canvas") offset exists before accessing left
        const canvasOffset = $("canvas").offset();
        d = a.x + d * a.w + (canvasOffset ? canvasOffset.left : 0);
...
Wijzer$$module$synpdf.prototype.setOffsetX = function() {
    ...
    if (1 < lpRec$$module$synpdf.loopBtn) {
        a -= this.xoffset;
        // Check if atag and btag exist before accessing offset and css
        const atagElm = this.atag;
        const btagElm = this.btag;
        if (atagElm && atagElm.offset()) {
            var b = atagElm.offset().left - a;
            atagElm.css("left", b);
        }
        if (btagElm && btagElm.offset()) {
            b = btagElm.offset().left - a;
            btagElm.css("left", b);
        }
    }
...
Wijzer$$module$synpdf.prototype.time2x = function(a) {
    var b, c;
    this.cursorTime = a;
    // Check if deTijden$$module$synpdf exists and has elements
    if (!deTijden$$module$synpdf || deTijden$$module$synpdf.length === 0) {
        console.warn("deTijden is empty, cannot map time to x.");
        return; // Exit if timing data is missing
    }
    ...
    for (b = deTijden$$module$synpdf.length - 1; 0 <= b; --b) {
        ...
        if (!(d.t > a)) {
            ...
            if (!opt$$module$synpdf.synbox && detix$$module$synpdf == deTijden$$module$synpdf.length - 1 && !m1_timer$$module$synpdf) {
                ...
                // Check if msc_wz$$module$synpdf exists before calling goMsre
            ...
            // Check if deMaten$$module$synpdf exists and has element at demix$$module$synpdf
            if (deMaten$$module$synpdf && deMaten$$module$synpdf.length > demix$$module$synpdf && (c = deMaten$$module$synpdf[demix$$module$synpdf])) {
                ...
                // Removed updating #detix-box and #demix-box as they are removed
                ...
                // Check if maatloper exists before accessing style
                if (this.maatloper && this.maatloper[0]) {
...
// Removed toggleScoreBtn$$module$synpdf as file buttons are removed
...
function countVsys$$module$synpdf(a, b, c) {
    ...
    for (f = 0; f < a.length; ++f) {
        var l = a[f][0];
        var h = a[f][a[f].length - 1];
        var k = [];
        for (d = 0; d < b; d += 4) {
            var r = 0;
            for (e = l * b + d; e < h * b + d; e += b) r += c[e], r += c[e + 1], r += c[e + 2];
            k.push(r / (3 * (h - l)))
        }
        for (d = r = 0; d < k.length; ++d) k[d] > r && (r = k[d]);
        witArr$$module$synpdf[f] = r * opt$$module$synpdf.zwgrens;
        d = Math.floor(k.length / 2);
        e = d + d / 2;
        for (l = 0; d < e; d++) h = k[d], h > r - 10 && (l += 1);
        if (!(5 < l) || opt$$module$synpdf.eerst) {
            p.push(a[f]);
            h = [];
            for (d = 0; d < k.length;)
                if (k[d] > r - 15) d += 1;
    ...
    for (f = 0; f < a.length - 1; ++f) {
        ...
        for (d = 0; d < b; d += 4) {
            r = 0;
            for (e = h * b + d; e < l * b + d; e += b) r += c[e], r += c[e + 1], r += c[e + 2];
            k.push(r / (3 * (l - h)))
        }
        e = k[0];
        for (d = r = 0; d < k.length; d++) h = k[d], e = Math.abs(h - e), 10 < e && e > r && (r = e), e = h;
        0 < r && m.push(r)
    }
    f = m.slice();
    f.sort(function(a, b) {
    ...
    for (r = 1; r < f.length - 1; ++r) k = f[r], k - b > c - k ? h.push(k) : d.push(k), b = d.reduce(function(a, b) {
        return a + b
    }, 0) / d.length, c = h.reduce(function(a, b) {
        return a + b
    }, 0) / h.length;
    d = c > opt$$module$synpdf.drmpl2 * b && 5 * h.length > d.length && !opt$$module$synpdf.onestf;
    for (f = 0; f < a.length - 1; ++f) k = m[f], d & k - b > c - k || 0 == opt$$module$synpdf.drmpl2 ? a[f + 1] = a[f].concat(a[f + 1]) : p.push({
        cs: a[f],
        xs: n[f]
    });
    p.push({
...
// Removed readDbxFile$$module$synpdf as Dropbox is removed
...
function readPdfOrJs$$module$synpdf(a) {
    ...
    // Removed check for impbox as import is removed
...
function readLocalFile$$module$synpdf(file = null) {
    ...
    // Use the provided file object or get it from the input element
...
// Removed readMedia$$module$synpdf as local media upload is removed
...
function medbtnSwitch$$module$synpdf() {
    // Only need to toggle YouTube related visibility based on checkbox
    var useYoutube = $("#yubuse").prop("checked");
    // Show/hide the YouTube input label and container
    $("#yvdlbl, #yubfile").css("display", useYoutube ? "block" : "none");
    // Ensure the main media buttons container is visible if YouTube is used
    $("#medbts").css("display", useYoutube ? "flex" : "none"); // Assuming you want it to be flex when visible
}
...
function setPlayer$$module$synpdf(a, b) {
    ...
    if (a) {
        ...
        a.on("timeupdate", function() {
            hasMixer$$module$mixer ? .5 < Math.abs(mixplayer$$module$mixer.currentTime - elmed$$module$synpdf.currentTime) &&
...
function setNotationHeight$$module$synpdf() {
    ...
    // Removed play button toggle
    // $("#knop").toggle(!!opt$$module$synpdf.playbtn);
    var a = parseFloat($("#buttons").css("height"));
    $("#vidyub").css("width", (1.52 * a).toFixed()); // Keep YouTube player width adjustment
    msc_wz$$module$synpdf && msc_wz$$module$synpdf.setTmargin()
}

function msc_shift$$module$synpdf(a) {
    // Keep this function as it controls the height of the top buttons section
...
function keyDown$$module$synpdf(a) {
    ...
    switch (b) {
        ...
        case "Down":
            ...
            break;
        case "PageUp":
        case "PageUp": // Added duplicate for clarity, can remove one
            msc_wz$$module$synpdf.goUpDown(0, 1, a);
            break;
        case "PageDown":
        case "PageDown": // Added duplicate for clarity, can remove one
        ...
        case " ":
            ...
            break;
        // Removed case "f" as file buttons are removed
        // case "f":
        //     $("#btns").click();
        //     break;
        case "h":
...
function msc_preload$$module$synpdf(a) {
    ...
    if (d.length > 1) {
        ...
        for (c = 0; c < d.length; c++) {
            ...
            // Check for PDF or JPG - Keep this logic
            if (/\.(pdf|jpg)$/.test(b)) {
                pdf_file$$module$synpdf = b;
                b = "";
            }

            // Check for audio/video - Removed local media check
            /*
    ...
    // Updated condition to reflect removed file types
    ...
    // Removed media_file$$module$synpdf from return condition
    return b || pdf_file$$module$synpdf || msc_tracks$$module$synpdf;
}
...
function msc_check_preload$$module$synpdf() {
    if (0 == playLstIx$$module$synpdf && play_list$$module$synpdf) $("body").trigger("play_end");
        ...
        // Removed local media file check and setPlayer call
        /*
...
function resetIntf$$module$synpdf(a) {
    ...
    */
    ...
    // Removed toggleBtns$$module$synpdf as file buttons are removed
    // toggleBtns$$module$synpdf();
    syncChk$$module$synpdf();
    hideSpeedChk$$module$synpdf();
    // Removed #err from visibility toggle
    $("#sync, #medbts").css("visibility", "visible");
...
function toggleBtns$$module$synpdf() {
    ...
    // Removed line accessing #btns as it's removed from HTML
...
function hideSpeedChk$$module$synpdf() {
    ...
    // Keep spdlbl toggle as the speed control is still present in the menu
...
// Removed loaddrop$$module$synpdf and dropuse$$module$synpdf as Dropbox is removed
/*
...
$(document).ready(function() {
    ...
    // Removed local file input change listener
    /*
    ...
    */
    // Removed local media input change listener
    ...
    // Removed play button click listener
    /*
    $("#knop").click(function() {
        keyDown$$module$synpdf({
            key: " "
    ...
    */
    ...
    // Corrected to use .val()
    let drmplValue = parseFloat($('#menu input#drmpl').val());
    ...
    Object.defineProperty(opt$$module$synpdf, 'drmpl', {
        ...
        set: function(newValue) {
            drmplValue = newValue;
            // Corrected to use .val()
            $('#menu input#drmpl').val(newValue);
        }
    });
    //Adding Cluster Threshold control SET and GET
    // Corrected to use .val()
    let drmpl2Value = parseFloat($('#menu input#drmpl2').val());
    ...
    Object.defineProperty(opt$$module$synpdf, 'drmpl2', {
        ...
        set: function(newValue) {
            ...
            // Corrected to use .val()
            $('#menu input#drmpl2').val(newValue);
        }
    });
    //Adding Barline Threshold control SET and GET
    // Corrected to use .val()
    let mtdrmplValue = parseFloat($('#menu input#mtdrmpl').val());
    ...
    Object.defineProperty(opt$$module$synpdf, 'mtdrmpl', {
        ...
        set: function(newValue) {
            ...
            // Corrected to use .val()
            $('#menu input#mtdrmpl').val(newValue);
        }
    });
});
var module$synpdf = {
