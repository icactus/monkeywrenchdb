...
var msc_VERSION$$module$synpdf = 182,
    ...
    onYouTubeAPIContinue$$module$synpdf, opt_default$$module$synpdf = {
        speed: 1,
        no_menu: 0,
        btns: 1,
        spdctl: 1,
...
function Wijzer$$module$synpdf(a, b, c, d) {
    ...
    $("#notation").append(b);
    this.maatloper = $('<div class="demaat" style="background:rgba(0,0,255,0.2); left:0px; top:0px; width:0px; height:0px"></div>');
...
function doeRol$$module$synpdf(a, b) {
    0 > a ? (a = deMaten$$module$synpdf[demix$$module$synpdf] || deMaten$$module$synpdf[0], deNot$$module$synpdf.scrollTop = 0, $("#rollijn").css("top", a.y + $("#notation").offset().top - dottedHeight$$module$synpdf - 1), msc_wz$$module$synpdf && msc_wz$$module$synpdf.setTmargin()) : (a = Math.round(a), deNot$$module$synpdf.scrollTop != a && (hasSmooth$$module$synpdf && (deNot$$module$synpdf.style["scroll-behavior"] = b ? "auto" : "smooth"), b || hasSmooth$$module$synpdf ? deNot$$module$synpdf.scrollTop = a : $(deNot$$module$synpdf).animate({
        scrollTop: a
    })))
}
...
function readPdfdoc$$module$synpdf() {
    ...
    pdfDoc$$module$synpdf.src ? setTimeout(function() {
...
function readPdf$$module$synpdf(a, b) {
    ...
    var c = a,
        d;
    "url" == b && (d = /jpe?g$/i.test(c));
    "pdfbin" == b && (a = new Uint8Array(a));
    "jpgbin" == b && (jpgData$$module$synpdf = new Uint8Array(a), b = "url", c = new Blob([a], {
        type: "image/jpeg"
    }), c = URL.createObjectURL(c));
    "url" == b && (d || /^blob:/.test(c)) ? (pdfDoc$$module$synpdf = new Image, pdfDoc$$module$synpdf.crossOrigin = "anonymous", pdfDoc$$module$synpdf.src = c, pdfDoc$$module$synpdf.onload = function() {
...
function goPage$$module$synpdf(a, b) {
    ...
    pdfDoc$$module$synpdf.getPage(a).then(function(c) {
        $("#render").html("rendering page: " + a);
        var d = c.getViewport(deMetriek$$module$synpdf[0] / c.pageInfo.view[2]),
        ...
        c.render({
            ...
            opt$$module$synpdf.advncd ? (rendering$$module$synpdf = 0, addDummySys$$module$synpdf()) :
                doresize$$module$synpdf ? resizePdf$$module$synpdf() : a < pdfDoc$$module$synpdf.numPages ? goPage$$module$synpdf(a + 1, b + e.height) : (rendering$$module$synpdf = 0, addDummySys$$module$synpdf())
        })
    })
}

function compPage$$module$synpdf(a, b, c) {
    // Assigns the value of deMetriek$$module$synpdf at index b to variable d
    var d = deMetriek$$module$synpdf[b];

    // Checks if d is falsy or if opt$$module$synpdf.advncd is truthy and pageNumChanged$$module$synpdf is falsy
    if (!d || opt$$module$synpdf.advncd && !pageNumChanged$$module$synpdf) {
        // If the condition is met, calls the countPix$$module$synpdf function with arguments a and opt$$module$synpdf.seln parsed to integer
        d = countPix$$module$synpdf(a, parseInt(opt$$module$synpdf.seln));
        
        // Checks if the length of d.cxs is 0
        if (0 == d.cxs.length) 
            // If it is, returns an object with property 'height' set to 0
            return {
                height: 0
            };
        
        // Assigns the value of d to deMetriek$$module$synpdf at index b
        deMetriek$$module$synpdf[b] = d;
    }

    // Sets pageNumChanged$$module$synpdf to 0
    pageNumChanged$$module$synpdf = 0;

    // Calls the knip$$module$synpdf function with arguments a, d, and c, and assigns the result to a
    a = knip$$module$synpdf(a, d, c);

    // Adds the value of Cs$$module$synpdf.length to the pageStfIx$$module$synpdf array
    pageStfIx$$module$synpdf.push(Cs$$module$synpdf.length);

    // Concatenates the d.cxs array to the Cs$$module$synpdf array
    Cs$$module$synpdf = Cs$$module$synpdf.concat(d.cxs);

    // If msc_wz$$module$synpdf is falsy, calls the startIntf$$module$synpdf function with argument a
    msc_wz$$module$synpdf || startIntf$$module$synpdf(a);

    // Appends a to the HTML element with id "notation"
    $("#notation").append(a);

    // Adds a mousedown and touchstart event listener to a, which calls the kliklang$$module$synpdf function
    $(a).on("mousedown touchstart", kliklang$$module$synpdf);

    // If the length of deMaten$$module$synpdf is greater than or equal to demix$$module$synpdf and msc_wz$$module$synpdf.cursorTime is truthy,
    // calls the msc_wz$$module$synpdf.time2x function with argument msc_wz$$module$synpdf.cursorTime
    ...
    // Calls the maatStrepen$$module$synpdf function
...
function goJpeg$$module$synpdf(a) {
...
function countPix$$module$synpdf(a, b) {
    ...
    var g = 4 * f;
    var p = a.height;
    a = d = a.getContext("2d").getImageData(0, 0, f, p).data;
    var m = 0;
    var n = [];
    var l = 3 * g / 4,
        h = g;
    opt$$module$synpdf.eerst && (l = 0, h = g / 4);
    for (e = 0; e < p; e++) {
        var k = 0;
        for (c = m + l; c < m + h; c += 4) k += d[c], k += d[c + 1], k += d[c + 2];
        c = k / (3 * (h - l));
        n.push(c);
        m += g
    }
    f = drawRes$$module$synpdf(n, f, p);
    for (f = countVsys$$module$synpdf(f, g, a); f.length && skipn$$module$synpdf;) f.shift(), --skipn$$module$synpdf;
    b && (f = f.slice(b - 1, b));
    b = findBarLines$$module$synpdf(f, g,
        a);
    return {
        cxs: f,
        bxs: b
...
function setPlayer$$module$synpdf(a, b) {
    ...
    if (a) {
        ...
        setNotationHeight$$module$synpdf()
    } else yubchk$$module$synpdf = 1, opt$$module$synpdf.media_height || (opt$$module$synpdf.media_height = "30%"), $("#vid, #aud").css("display", "none"), $("#vidyub").css("display", "inline-block"), yubload$$module$synpdf(function() {
        elmed$$module$synpdf = ybplayer$$module$synpdf;
        pbrates$$module$synpdf = elmed$$module$synpdf.getAvailablePlaybackRates();
        setSpeed$$module$synpdf(0);
        setNotationHeight$$module$synpdf();
