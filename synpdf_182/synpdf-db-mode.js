...
function goPage$$module$synpdf(a, b) {
    ...
    pdfDoc$$module$synpdf.getPage(a).then(function(c) {  // Get a Promise that can access the specific page of the PDF document
        $("#render").html("rendering page: " + a + "/" + pdfDoc$$module$synpdf.numPages);  // Update the HTML element with id='render' with the page number
        var d = c.getViewport(deMetriek$$module$synpdf[0] / c.pageInfo.view[2]);  // Set the scale for the page rendering, based on device resolution
        var e = document.createElement("canvas");  // Create a canvas element
        var f = e.getContext("2d");  // Get its 2D rendering context
        e.height = d.height;  // Set the dimensions of the canvas to match the page size
        e.width = d.width;
        c.render({  // Render the PDF page into the canvas
            canvasContext: f,
            viewport: d
...
function countPix$$module$synpdf(a, b) {
    ...
    var f = a.width;  // Get the width of the image
    var g = 4 * f;  // Multiply width by 4 for later calculations
    var p = a.height;  // Get the height of the image
    a = d = a.getContext("2d").getImageData(0, 0, f, p).data;  // Extract the image data (RGBA values for each pixel)
    ...
    for (e = 0; e < p; e++) {  // Go through each row in the image
    ...
    f = drawRes$$module$synpdf(n, f, p);  // Draw the results of the pixel analysis
    for (f = countVsys$$module$synpdf(f, g, a); f.length && skipn$$module$synpdf;) f.shift(), --skipn$$module$synpdf;  // Further process the image data
    b && (f = f.slice(b - 1, b));  // Slice the results based on parameter 'b'
    b = findBarLines$$module$synpdf(f, g, a);  // Find bar lines
    return {
        cxs: f,  // Return the results
        bxs: b
    }
}
    ...
    b.forEach(function(a, b) {
        ...
        switch (a) {
            ...
            case "times_arr":
                ...
                break;
            case "metric_arr":
                metric_arr$$module$synpdf = 1; // setting as 1 allows override in msc_check_preload$$module$synpdf
...
function msc_check_preload$$module$synpdf() {
    if (0 == playLstIx$$module$synpdf && play_list$$module$synpdf) $("body").trigger("play_end");
        ...
        //IF metric_arr is true, then we get the metric_arr from the db. Later figure out how to remove this "if true" part.
        metric_arr$$module$synpdf && fetchDataFromDB()
        .then(function(response) {
          deMetriek$$module$synpdf = response;
          jsonString = JSON.stringify(deMetriek$$module$synpdf);
          localStorage.setItem('jsonString', jsonString);
          schaalMetriek$$module$synpdf();
        })
        .catch(function(error) {
          // Handle any errors that occurred during the fetchDataFromDB() function
          console.error(error);
        });
