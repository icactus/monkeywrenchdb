...
function fetchSearchByInstrument() {
  $.ajax({
    url: './phpfiles/fetchinstruments_data.php',
    method: 'GET',
    ...
    success: function(response) {
      ...
      if (response === "No pieces found for the selected instrument") {
        ...
        pieces.forEach(function(piece) {
          container.append('<a href="#" class="pieces-link" data-id="' + piece.metric_arr_id + '" data-piece-id="' + piece.piece_id + '" data-instrument-id="' + instrumentId +'">' + piece.composer_last + ' ' + piece.piece_name + '</a><br>');
        });
      }
    }
  });
...
function generateInstrumentsDropdown(recordingId) {
  return new Promise(function(resolve, reject) {
    ...
    xhr.open("GET", "./phpfiles/get_recording_instruments.php?recordingId=" + recordingId, true);
...
function fetchRecordings(metricArrId) {
  return new Promise(function(resolve, reject) {
    $.ajax({
      url: './phpfiles/fetchrecordings_data.php',
      method: 'GET',
      ...
      success: function(response) {
        ...
        if (response === "No recordings found for the selected piece") {
          ...
          recordings.forEach(function(recordingFullData) {
            var link = $('<a href="#" class="recordings-link">' + recordingFullData.conductor_name + ' - ' + recordingFullData.ensemble_name + '</a><br>');
            link.data('recordingFullData', recordingFullData); // Attach the recording data to the link
          ...
          var metricArrData = $(recordingsDropdown.find('option')[1]).data('recordingFullData').metric_arr_data;
          deMetriek$$module$synpdf = metric_arr$$module$synpdf = JSON.parse(metricArrData);
...
// REMEMBER TO FIX THIS SO THAT IT DOESN'T DOWNLOAD TIMES ARRAY FOR EVERY RECORDING IN ADVANCE
...
function fetchNewInstrument(instrumentData) {
  return new Promise((resolve, reject) => {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "./phpfiles/get_new_instrument_data.php?recordingId=" + recordingId + "&metricId=" + metricId, true);
    xhr.onreadystatechange = function() {
...
function fetchNewRecording() {
  return new Promise((resolve, reject) => {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "./phpfiles/fetch_new_recording.php?recordingId=" + currentRecordingGlobal + "&InstrumentId=" + currentInstrumentGlobal, true);
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4 && xhr.status === 200) {
...
function fetchAndLoadRecording() {
  fetchNewRecording()
    .then(newRecordingData => {
      ...
      // speed was changing to 0.25 because opt$$module$synpdf.speed ends up becoming undefined on recording change. manual fix for now.
...
$('#pieces-container').on('click', '.pieces-link', function() {
  ...
  //set up pdf in advance
  var pieceId = $(this).data('piece-id');
  var instrumentId = $(this).data('instrument-id');
  pdf_file$$module$synpdf = "./pdfs/" + pieceId + "-" + instrumentId + ".pdf";
...
$('#recordings-container').on('click', '.recordings-link', function() {
  ...
  .then(function() {
    ...
    //THIS RELOADS THE FILE NEEDLESSLY. WE JUST NEED TO SET UP THE PLAYER SINCE FILE IS ALREADY LOADED IN BACKGROUND
    setPlayer$$module$synpdf("", "");
    fetchAndLoadRecording();
    $("#sidecontent").show();
    $("#search-area").hide();
...
// document.getElementById('speed-increment').addEventListener('click', function(){
//   var input = document.getElementById('speed');
//   if (input.value < input.max) {
//       input.value = Number((parseFloat(input.value) + parseFloat(input.step)).toFixed(2));
//   }
// });

// document.getElementById('speed-decrement').addEventListener('click', function(){
//   var input = document.getElementById('speed');
